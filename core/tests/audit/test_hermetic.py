...
def test_replay_determinism(temp_storage):
    ...
    res = mem1.record_decision(title="D1", target="T1", rationale="R1")
    
    # Add episodic event and link it
    event = MemoryEvent(source="agent", kind="result", content="Evidence")
...
def test_trust_boundary_bypass_attempt(temp_storage):
    ...
    event = MemoryEvent(
        ...
        context={"title": "Hack", "target": "T", "rationale": "R"}
    )
    ...
    with pytest.raises(PermissionError) as excinfo:
        memory.semantic.update_decision("any.md", {"status": "superseded"}, "Hacked Update")
    assert "Trust Boundary Violation" in str(excinfo.value)
...
def test_false_conflict_on_load(temp_storage):
    ...
    memory.record_decision(title="Old", target="Database", rationale="Old rationale")
    files = memory.get_decisions()
    fid1 = files[0]
    
    time.sleep(1.1)
    # Supersede it
    memory.supersede_decision(title="New", target="Database", rationale="New rationale", old_decision_ids=[fid1])
...
def test_negative_capability_empty_fields(memory):
    ...
    with pytest.raises(ValidationError):
        memory.record_decision(title="Test", target="  ", rationale="Something")
        
    # Empty rationale
    with pytest.raises(ValidationError):
        memory.record_decision(title="Test", target="T", rationale="")
