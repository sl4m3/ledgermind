...
from application.errors import IdempotencyConflict, UnsupportedEvolutionDecision
from domain import (
    Atom,
    EvidenceRelation,
    KnowledgeEvidence,
...
from domain.policies import CreateNewPattern, KnowledgeEvolutionPolicy
from ports import Clock, IdentifierFactory, UnitOfWork
from ports.repository_ports import DomainEvent, StoredIdempotencyResult
...
class IngestAtomHandler:
    ...
    def handle(self, command: IngestAtomCommand) -> IngestAtomResult:
        with self._uow_factory() as uow:
            stored = uow.idempotency.get(command.idempotency_key)
