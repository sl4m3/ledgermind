...
export function activate(context: vscode.ExtensionContext) {
            ...
            chat.onDidReceiveChatResponse((e: any) => {
                ...
                execFile('ledgermind-mcp', [
                    ...
                    if (err) {
                        ...
                        vscode.window.showErrorMessage(`LedgerMind Sync Error: ${err.message || err}`);
                    }
                });
            })
        );
    ...
    const updateShadowContext = async (prompt?: string) => {
        ...
        execFile('ledgermind-mcp', [
            ...
            setBusy(false);
            if (err) {
                console.error('LedgerMind Context Update Error:', err);
                vscode.window.showErrorMessage(`LedgerMind Sync Error: ${err.message || err}`);
            } else if (stdout) {
    ...
    context.subscriptions.push(
        vscode.workspace.onDidSaveTextDocument(doc => {
            ...
            execFile('ledgermind-mcp', [
                ...
                if (err) {
                    console.error('LedgerMind Save Record Error:', err);
                    vscode.window.showErrorMessage(`LedgerMind Sync Error: ${err.message || err}`);
                }
            });
        }),
