...
export function activate(context: vscode.ExtensionContext) {
                ...
                debounceTimer = setTimeout(() => {
                    ...
                    if (cmds) {
                        execFile('ledgermind-mcp', [
                            ...
                            if (err) vscode.window.showErrorMessage('LedgerMind Terminal Watch Error: ' + err.message);
                        });
                    }
                    terminalBuffer = '';
                }, 1500); // Debounce 1.5s
    ...
    const updateShadowContext = async (prompt?: string) => {
        ...
        execFile('ledgermind-mcp', [
            ...
            setBusy(false);
            if (err) {
                vscode.window.showErrorMessage('LedgerMind Context Injection Error: ' + err.message);
            } else if (stdout) {
    ...
    context.subscriptions.push(
        vscode.workspace.onDidSaveTextDocument(doc => {
            ...
            execFile('ledgermind-mcp', [
                ...
                if (err) vscode.window.showErrorMessage('LedgerMind Document Save Error: ' + err.message);
            });
        }),
