...
class QueryService(MemoryService):
    ...
    def generate_knowledge_graph(self, target: Optional[str] = None) -> str:
        ...
        generator = KnowledgeGraphGenerator(
            self.semantic.repo_path, self.semantic.meta, self.episodic
        )
    ...
    def search(
        ...
        if mode == "lite" or (is_short_query and mode == "balanced"):
            ...
            kw_results = self.semantic.meta.keyword_search(
                query, limit=limit, namespace=effective_namespace, status=search_status
            )
            if kw_results:
                fast_results = [
                    {
        ...
        search_limit = (
            max(200, (offset + limit) * 10) if namespace else (offset + limit) * 3
        )
        ...
        kw_results = self.semantic.meta.keyword_search(
            query, limit=search_limit, namespace=effective_namespace
        )

        all_initial_fids = list(
            set(
                [item.get("id") for item in vec_results]
                + [r.get("fid") for r in kw_results]
            )
        )
        meta_cache = {
            m["fid"]: m for m in self.semantic.meta.get_batch_by_fids(all_initial_fids)
        }
        ...
        current_layer_ids = [
            m["superseded_by"] for m in candidates_meta if m.get("superseded_by")
        ]
        current_layer_ids = [
            fid for fid in current_layer_ids if fid and fid not in request_cache
        ]
        ...
        while current_layer_ids and iteration < 5:
            ...
            current_layer_ids = [
                m["superseded_by"] for m in new_batch if m.get("superseded_by")
            ]
            current_layer_ids = [
                fid for fid in current_layer_ids if fid and fid not in request_cache
            ]
        ...
        for fid in sorted_fids:
            ...
            if not self.context.include_history and status not in (
                "active",
                "superseded",
                "deprecated",
                "pending_merge",
        ...
        for fid, meta, match_score in resolved_records:
            ...
            lifecycle_multiplier = self._get_lifecycle_multiplier(
                phase, vitality, kind, meta.get("status")
            )
        ...
        for cand in final_candidates.values():
            raw_score = (cand["base_score"] + cand["boost"]) * cand[
    ...
    def _resolve_to_truth(
        self, doc_id: str, mode: str, cache: Optional[Dict[str, Dict[str, Any]]] = None
    ...
    def _get_lifecycle_weight(self, meta: Optional[Dict[str, Any]]) -> float:
        ...
        if vitality == "decaying":
            weight *= 0.5
        elif vitality == "dormant":
            weight *= 0.2
        return weight

    def _get_lifecycle_multiplier(
        self, phase: str, vitality: str, kind: str, status: Optional[str]
    ) -> float:
        ...
        if status in ("rejected", "falsified"):
            multiplier *= 0.2
