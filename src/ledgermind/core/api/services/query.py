...
class QueryService(MemoryService):
    ...
    def search(self, query: str, limit: int = 5, offset: int = 0,
        ...
        current_layer_ids = list({
            m['superseded_by'] for m in candidates_meta
            if m.get('superseded_by') and m['superseded_by'] not in request_cache
        })
        ...
        while current_layer_ids and iteration < 5:
            ...
            current_layer_ids = list({
                m['superseded_by'] for m in new_batch
                if m.get('superseded_by') and m['superseded_by'] not in request_cache
            })
