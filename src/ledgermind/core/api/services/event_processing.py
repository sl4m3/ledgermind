...
class EventProcessingService(MemoryService):
    ...
    def process_event(
        ...
        content: str,
        context: Optional[
            Union[DecisionContent, DecisionStream, Dict[str, Any]]
        ] = None,
        ...
        if (
            self.context.trust_boundary == TrustBoundary.HUMAN_ONLY
            and source == "agent"
            and kind == KIND_DECISION
        ):
        ...
        if (
            decision
            and decision.should_persist
            and decision.store_type == "semantic"
            and not intent
        ):
        ...
        if decision and decision.should_persist:
            if decision.store_type == "episodic":
                if source in {"user", "agent"}:
                    ...
                    if event_emitter:
                        event_emitter.emit(
                            "episodic_added", {"id": ev_id, "kind": event.kind}
                        )
    ...
    def _handle_intervention(
        self, content: str, context: Any, namespace: str
    ) -> DecisionStream:
        stream = DecisionStream(
            decision_id=(
                context.get("decision_id", str(uuid.uuid4()))
                if isinstance(context, dict)
                else getattr(context, "decision_id", str(uuid.uuid4()))
            ),
            target=(
                context.get("target", "unknown")
                if isinstance(context, dict)
                else getattr(context, "target", "unknown")
            ),
            title=(
                context.get("title", content)
                if isinstance(context, dict)
                else getattr(context, "title", content)
            ),
            rationale=(
                context.get("rationale", content)
                if isinstance(context, dict)
                else getattr(context, "rationale", content)
            ),
            namespace=namespace or self.context.namespace,
        )
        return self.context.reflection_engine.lifecycle.process_intervention(
            stream, datetime.now()
        )
    ...
    def _persist_semantic(
        ...
        with self.transaction(description="Persist Semantic Record"):
            ...
            if intent and intent.resolution_type == "supersede":
                for old_id in intent.target_decision_ids:
                    ...
                    if old_meta and old_meta.get("status") in (
                        "active",
                        "pending_merge",
                        "accepted",
                        "draft",
            ...
            if (
                intent
                and intent.resolution_type == "supersede"
                and intent.target_decision_ids
            ):
            ...
            if event_emitter:
                event_emitter.emit(
                    "semantic_added",
                    {"id": new_fid, "kind": event.kind, "namespace": namespace},
                )
        ...
        is_merge = event.kind == KIND_DECISION and (
            (
                isinstance(event.context, dict)
                and event.context.get("target") == "knowledge_merge"
            )
            or (intent and intent.resolution_type == "supersede")
        )

        if (
            source in {"user", "agent"}
            and not (source == "agent" and is_merge)
            and source not in INTERNAL_SOURCES
        ):
            ...
            if linked_fid and self.query_service:
                truth = self.query_service._resolve_to_truth(
                    linked_fid, mode="balanced"
                )
                if truth and truth.get("fid") != linked_fid:
                    logger.info(
                        f"Forwarding event from {linked_fid} to active truth {truth.get('fid')}"
                    )
