...
from ledgermind.core.stores.interfaces import (
    MetadataStore,
    EpisodicProvider,
    AuditProvider,
)
...
class Memory:
    ...
    def __init__(
        ...
        if config:
            ...
            self.config = LedgermindConfig(
                ...
                or "../.ledgermind/models/v5-small-text-matching-Q4_K_M.gguf",
        ...
        try:
            ...
            raise ValueError(
                f"No permission to create storage path: {self.storage_path}"
            )

        # 1. Initialize Storage (Metadata must be ready first)
        if semantic_store:
            self.semantic = semantic_store
            self.episodic: Union[EpisodicStore, EpisodicProvider] = (
                episodic_store
                or EpisodicStore(os.path.join(self.storage_path, "episodic.db"))
            )
        else:
        ...
        self.conflict_engine = ConflictEngine(
            self.semantic.repo_path, meta_store=self.semantic.meta
        )
        ...
        self.reflection_engine = ReflectionEngine(
            self.episodic, self.semantic, processor=self
        )
        ...
        self._event_processing = EventProcessingService(
            self.context, query_service=self._query
        )
    ...
    def update_decision(
        ...
        """
        return self._decision_command.update_decision(
            decision_id, updates, commit_msg, skip_episodic
        )
