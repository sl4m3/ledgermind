...
class CloudLLMClient:
    ...
    def _call_cli(self, instructions: str, data: str, fid: str) -> Optional[str]:
        try:
            ...
            logger.info(f"{self._bin.capitalize()} CLI: Calling model {self.config.model_name} for {fid}...")
            ...
            for attempt in range(1, self.config.retry_attempts + 1):
                logger.info(f"Attempt {attempt}/{self.config.retry_attempts}: {self._bin.capitalize()} CLI call...")
                try:
