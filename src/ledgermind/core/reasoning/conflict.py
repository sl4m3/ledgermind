...
class ConflictEngine:
    ...
    def get_conflict_files(self, event: MemoryEvent, namespace: Optional[str] = None) -> List[str]:
        ...
        if self.meta:
            ...
            # ⚡ Bolt: Use SQL-optimized list_active_conflicts instead of list_all + Python filtering
            # to avoid instantiating dictionaries and let SQLite filter efficiently
            if hasattr(self.meta, 'list_active_conflicts'):
                return self.meta.list_active_conflicts(new_target, namespace=ns)
            else:
                # Fallback if mock/alternative meta store is used
                metas = self.meta.list_all(target=new_target, namespace=ns)
                return [m['fid'] for m in metas if m.get('status') == 'active']
