...
class VectorEmbeddingAlgorithm(DuplicateSearchAlgorithm):
    ...
    def _keyword_semantic_similarity(
        self,
        doc1: Dict[str, Any],
        doc2: Dict[str, Any],
    ...
    def calculate_similarity(
        ...
        logger.debug(
            f"Quadratic Cluster: sem={semantic_sim:.4f}, kw={kw_sim:.4f}, target={target_sim:.4f} -> combined={combined:.4f}"
        )
    ...
    def search(self, candidate: Dict[str, Any], memory: Any) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if not query.strip():
                ...
                all_docs = memory.search_decisions(
                    query, limit=self.vector_search_limit, mode="maintenance"
                )
            elif hasattr(memory, "_query") and hasattr(memory._query, "search"):
                all_docs = memory._query.search(
                    query, limit=self.vector_search_limit, mode="maintenance"
                )
            else:
        ...
        logger.info(
            f"Search complete in {elapsed:.3f}s, found {len(results)} duplicates"
        )
        return results

    def _is_near_identical(
        self, text1: str, text2: str, threshold: float = 0.95
    ) -> bool:
