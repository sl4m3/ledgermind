"""Contract tests for atom ingestion v1."""
...
def test_valid_ingest_atom_request_v1() -> None:
    request = _fixture_request()
    parsed = IngestAtomRequestV1.model_validate(request)
...
def test_unknown_root_field_is_rejected() -> None:
    ...
    with pytest.raises(pydantic.ValidationError):
        IngestAtomRequestV1.model_validate(request)
...
def test_phase_status_confidence_fields_are_rejected() -> None:
    ...
    with pytest.raises(pydantic.ValidationError):
        IngestAtomRequestV1.model_validate(request2)
...
def test_no_circular_import_between_contract_modules() -> None:
    import ledgermind_core.contracts.atom_v1 as atom_v1
    import ledgermind_core.contracts.common as common

    if not common.__file__ or not atom_v1.__file__:
        raise AssertionError("contract module file path missing")

    atom_source = Path(atom_v1.__file__).read_text(encoding="utf-8")
    common_source = Path(common.__file__).read_text(encoding="utf-8")

    assert "from .atom_v1 import" not in common_source
    assert "from .common import" not in atom_source
    assert "from .atom_v1 import" not in atom_source

    assert atom_v1 is not None
...
def test_schema_can_be_exported_and_matches_snapshot() -> None:
    snapshot_path = (
        ...
        / "contracts"
        / "snapshots"
        / "ingest_atom_v1.schema.json"
    )
    expected = json.loads(snapshot_path.read_text(encoding="utf-8"))
    exported = IngestAtomRequestV1.model_json_schema()
