...
def test_mode_optimal_calls_local_llm(sample_proposal):
    """Verify that enrichment calls the unified BaseURLClient."""
    ...
    with patch("ledgermind.core.reasoning.enrichment.base_client.BaseURLClient.call", return_value=json.dumps(valid_json)):
...
def test_rich_mode_calls_cloud_model(sample_proposal):
    """Verify that enrichment triggers the unified client path."""
    enricher = LLMEnricher()
